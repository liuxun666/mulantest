...
public class BPMLL extends MultiLabelLearnerBase {
    ...
    public void build(MultiLabelInstances trainingSet) throws Exception {
        super.build(trainingSet);
    }

    public void build(List<DataPair> trainData, int numLabels, int[] labelIndices, String[] labelNames, int[] featureIndices) throws Exception {
        this.numLabels = numLabels;
        this.labelIndices = labelIndices;
        this.labelNames = labelNames;
        this.featureIndices = featureIndices;
        buildInternal(trainData);
    }



    protected void buildInternal(List<DataPair> trainData) throws Exception{
    ...
    public MultiLabelOutput predict(double[] data){
        double[] labelConfidences = model.feedForward(data);
    ...
    public MultiLabelOutput predict(DataPair dp) {
        double[] labelConfidences = model.feedForward(dp.getInput());
        double threshold = thresholdF.computeThreshold(labelConfidences);
        boolean[] labelPredictions = new boolean[numLabels];
        Arrays.fill(labelPredictions, false);
    ...
    public double[][] getWights() {
        List<Neuron> units = model.getLayerUnits(model.getLayersCount() - 2);
        double[][] weigths = new double[units.size()][];
        for (int i = 0; i < units.size(); i++){
            weigths[i] = units.get(i).getWeights();
        }
        return weigths;
    }
}